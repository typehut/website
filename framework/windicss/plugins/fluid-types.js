...
const fluidTypePlugin = plugin(
  function ({ addUtilities, theme, variants, e }) {
    ...
    const settingsAsArray = Object.entries(
      theme("fluidType.settings", DEFAULT_SETTINGS)
    );
    ...
    const calcModularScale = (value = 0) => {
      if (settingsAreNumbers) {
        const sFtMin = theme(
          "fluidType.settings.fontSizeMin",
          DEFAULT_SETTINGS.fontSizeMin
        );
        const sFtMax = theme(
          "fluidType.settings.fontSizeMax",
          DEFAULT_SETTINGS.fontSizeMax
        );
        const sFtRMin = theme(
          "fluidType.settings.ratioMin",
          DEFAULT_SETTINGS.ratioMin
        );
        const sFtRMax = theme(
          "fluidType.settings.ratioMax",
          DEFAULT_SETTINGS.ratioMax
        );
        const sFtSMin = theme(
          "fluidType.settings.screenMin",
          DEFAULT_SETTINGS.screenMin
        );
        const sFtSMax = theme(
          "fluidType.settings.screenMax",
          DEFAULT_SETTINGS.screenMax
        );
        const sFtUnit =
          typeof theme("fluidType.settings.unit", DEFAULT_SETTINGS.unit) ===
          "string"
    ...
    addUtilities(
      Object.entries(values).map(([key, value]) => {
        ...
        if (Array.isArray(value) && value.length === 1) {
          output.fontSize = Number.isInteger(value[0])
            ? calcModularScale(value[0])
            : value[0];
        }
...
export default fluidTypePlugin;
